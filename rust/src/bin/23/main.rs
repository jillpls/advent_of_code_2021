...
fn main() {
    ...
    'outer: while let Some(current_index) = to_compute.pop_back() {
        ...
        for (s, new_cost) in new_states {
            ...
            game.game_tree[current_index].children.push(len);
            game.game_tree.push(GameTree {
                state: s,
                pos: len,
    ...
    fn min_cost(&self) -> u64 {
        ...
        for a in self.amphipods {
            if a.position.0 != a.target * 2 + 1 {
                cost +=
                    a.cost as u64 * ((a.target * 2 + 1) as i32 - a.position.0 as i32).abs() as u64;
    ...
    pub fn expand(&self, possible_targets: &HashSet<(usize, usize)>) -> Vec<(GameState, u64)> {
        ...
        for mut j in 0..2 {
            for (i, a) in amphipods.iter().enumerate() {
                ...
                for d in DIRECTIONS {
                    if is_passable(
                        map[(a.position.0 as i32 + d.0) as usize]
                            [(a.position.1 as i32 + d.1) as usize],
                    ) {
                        skip = false;
                    }
                }
...
fn get_cost(start: (usize, usize), end: (usize, usize), map: &[[char; 7]; 13]) -> Option<u32> {
    ...
    return Some(
        ((orig_start.1 - 1) + (orig_start.0 as i32 - end.0 as i32).abs() as usize + (end.1 - 1))
            as u32,
    );
}
