...
impl SFNumber {
    ...
    fn add_first_left(self, amount : i32) -> Self {
        match self {
            Self::Number(v) => {
                Self::Number(v + amount)
            },
            Self::Pair(v1, v2) => {
                Self::Pair(Box::from(v1.add_first_left(amount)), v2)
            }
        }
    }
    ...
    fn add_first_right(self, amount : i32) -> Self {
        match self {
            ...
            Self::Pair(v1, v2) => {
                Self::Pair(v1, Box::from(v2.add_first_right(amount)))
            }
        }
    }
...
fn parse_sf_number(chars : &mut std::str::Chars) -> SFNumber {
    ...
    let char = loop {
        let char = chars.next();
        if char.is_none() { panic!(); }
        if char.unwrap() == ',' { break char.unwrap(); }
    };
